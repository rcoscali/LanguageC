...
int main(int argc, char *argv[])
{
  ...
  qsort(tab,
        NB_ELEMENTS,
        sizeof(tab[0]),
        (int(*)(const void*, const void *))comp_str_maj);
  if ((res = bsearch(&argv[1],
                     tab,
                     NB_ELEMENTS,
                     sizeof(tab[0]),
                     (int(*)(const void*, const void *))comp_str_maj)) == NULL)
    printf("Le tableau ne contient pas l’element %s\n",argv[1]);
  else
    printf("Le tableau contient l’element %s sous la forme %s\n", argv[1], *res);
  return(EXIT_SUCCESS);
}
